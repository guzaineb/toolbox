...
import { Button, Card, Modal, EmptyState, Skeleton } from '@/components/shared/ui'
...
function Toast({ message, type, onClose }: { message: string; type: 'success' | 'error'; onClose: () => void }) {
  ...
  return (
    <div className={`fixed top-4 right-4 z-50 flex items-center gap-2.5 px-4 py-3 rounded-xl border shadow-elevated animate-slideIn ${bg}`}>
...
export default function ExpertDashboardPage() {
  ...
  const [expertiseToDelete, setExpertiseToDelete] = useState<{ areaId: string; areaName: string } | null>(null)
  ...
  if (!profile) {
    return (
      <EmptyState
        icon={<Target size={28} />}
        title="Créez votre profil expert"
        description="Devenez expert et soyez sollicité pour accompagner des projets innovants"
        action={
  ...
  return (
    <div className="max-w-7xl mx-auto py-8 px-4 animate-fadeIn">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          ...
          <Card className="p-6">
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-syne text-[16px] font-bold text-ink">Mes domaines d'expertise</h3>
            ...
            {totalExpertises === 0 ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {profile.expertiseConnections.map((conn) => (
                  <div key={conn.id} className="flex items-center justify-between p-3 bg-moss/[.03] rounded-lg border border-moss/10 hover:border-moss/20 transition-all">
                    <div>
                    ...
                    <div className="flex gap-2">
                      <button
                        ...
                        className="text-moss hover:text-moss/80 p-1.5 rounded-lg hover:bg-moss-light transition-all"
                      >
                      ...
                      <button
                        ...
                        disabled={saving}
                        className="text-red-500 hover:text-red-700 p-1.5 rounded-lg hover:bg-red-light transition-all"
                      >
                        <X className="w-4 h-4" />
                      </button>
                    </div>
        ...
        <div className="space-y-6">
          {score && (
            <div className="bg-gradient-to-br from-moss to-moss-dark text-white rounded-xl p-6 shadow-elevated">
              <div className="flex items-center justify-between mb-4">
                <div>
              ...
              <div className="w-full bg-white/20 rounded-full h-2 overflow-hidden">
                <div className="bg-white/80 h-2 rounded-full transition-all duration-700" style={{ width: `${score.score}%` }} />
              </div>
              <div className="mt-4 space-y-2 text-sm">
                {[
                  { label: 'Expérience', value: Math.round(score.details.experience.score), max: 30 },
                  { label: 'Diversité', value: Math.round(score.details.diversity.score), max: 20 },
                  { label: 'Niveaux', value: Math.round(score.details.levels.score), max: 30 },
                  { label: 'Disponibilité', value: Math.round(score.details.availability.score), max: 20 },
                ].map((item) => (
                  <div key={item.label} className="flex justify-between items-center">
                    <span className="opacity-80">{item.label}</span>
                    <div className="flex items-center gap-2">
                      <div className="w-20 h-1.5 bg-white/15 rounded-full overflow-hidden">
                        <div className="h-full bg-white/60 rounded-full" style={{ width: `${(item.value / item.max) * 100}%` }} />
                      </div>
                      <span className="font-medium tabular-nums w-8 text-right">{item.value}/{item.max}</span>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <Card className="p-6">
            <h3 className="text-sm font-semibold text-ink3 uppercase tracking-wider mb-4">Statistiques</h3>
            <div className="space-y-4">
              {[
                { icon: Users, label: 'Total expertises', value: totalExpertises },
                { icon: Star, label: 'Niveau moyen', value: `${avgLevel.toFixed(1)}/4` },
                { icon: Award, label: 'Statut', value: <AvailabilityBadge status={profile.availability_status} size="sm" showLabel={false} /> },
                { icon: Calendar, label: 'Membre depuis', value: new Date(profile.created_at || Date.now()).toLocaleDateString() },
              ].map((item) => (
                <div key={item.label} className="flex justify-between items-center">
                  <div className="flex items-center gap-2">
                    <item.icon className="w-4 h-4 text-ink3" />
          ...
          <Card className="p-6">
            <h3 className="text-sm font-semibold text-ink3 uppercase tracking-wider mb-4">Niveaux</h3>
                ...
                return (
                  <div key={level}>
                    <div className="flex justify-between text-sm mb-1">
                      <span className="capitalize text-ink2">{level}</span>
                      <span className="font-medium text-ink">{count}</span>
                    </div>
                    <div className="w-full bg-moss/10 rounded-full h-2 overflow-hidden">
                      <div className={`${colors[level]} h-2 rounded-full transition-all duration-500`} style={{ width: `${percent}%` }} />
                    </div>
                  </div>
                )
              })}
          ...
          <button
            ...
            className="w-full bg-surface hover:bg-moss-light/30 rounded-xl p-4 text-left transition-all border border-moss/20 hover:border-moss/30 group"
          >
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-ink group-hover:text-moss transition-colors">Changer ma disponibilité</p>
      ...
      <Modal
        ...
        onClose={() => { setShowDeleteConfirm(false); setExpertiseToDelete(null) }}
        title="Supprimer un domaine d'expertise"
      >
        <p className="text-sm text-ink2 mb-6">
          Êtes-vous sûr de vouloir supprimer le domaine :
          {' '}<span className="font-semibold text-ink">{expertiseToDelete?.areaName}</span> ?
        ...
        <div className="flex gap-3">
          <Button
            variant="secondary"
            fullWidth
            onClick={() => { setShowDeleteConfirm(false); setExpertiseToDelete(null) }}
          >
            Annuler
          </Button>
          <Button
