...
export default function VerifyEmailClient({
  ...
  const [status, setStatus] = useState<
    'verifying' | 'success' | 'error' | 'needCode'
  >('verifying');
  ...
  const handleResendCode = async () => {
    if (!email) {
      setErrorMessage(
        'Veuillez renseigner votre email pour recevoir un nouveau code.'
      );
      return;
    }
    ...
    if (status === 'success') {
      return (
        <div className="text-center">
          ...
          <h1 className="text-[20px] font-semibold mb-2 text-green-600">
            Email vérifié !
          </h1>
    ...
    if (status === 'error') {
      return (
        <div className="text-center">
          ...
          <h1 className="text-[20px] font-semibold mb-2 text-red-600">
