...
export class UsersService {
      ...
      data: {
        ...
        profile: {
          create: {
            ...
            birth_date: createUserDto.profile.birthDate ? new Date(createUserDto.profile.birthDate) : undefined,
  ...
  async findById(id: string) {
    return this.prisma.user.findUnique({
      where: { id },
      include: { profile: true, projectOwnerProfile: true, expertProfile: true, incubatorMembers: true },
    });
  }
...
async updateProfile(userId: string, data: UpdateProfileDto) {
  ...
  await this.prisma.userProfile.update({ where: { id: user.profile.id }, data: updateData });
