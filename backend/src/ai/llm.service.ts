  ...
  async chat(messages: LlmMessage[], options?: LlmOptions): Promise<LlmResponse> {
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
      try {
        ...
        this.logger.log(`Groq call completed in ${duration}ms, model: ${data.model}, tokens: ${data.usage?.total_tokens}`);
        ...
        lastError = error;
        this.logger.warn(`Groq API call attempt ${attempt}/3 failed: ${error.message}`);
    ...
    this.logger.error(`Groq API call failed after 3 attempts: ${lastError!.message}`);
    throw lastError;
  }
