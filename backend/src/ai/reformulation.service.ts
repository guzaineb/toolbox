...
const STEP_CONCEPTS: Record<string, string> = {
  gbm_1:
    "Esquisse d'idée — définition du concept de base, produit/service, clients et partenaires",
  gbm_2:
    'Problèmes et besoins — identification des défis environnementaux, sociaux et besoins clients',
  gbm_3:
    'Analyse PESTEL — compréhension du contexte politique, économique, social, technologique, environnemental et légal',
  gbm_4:
    "Objectifs — fixation d'objectifs environnementaux, sociaux, clients et équipe",
  gbm_5:
    'Mission et vision — synthèse de la mission, vision et valeurs du projet',
  gbm_6: 'Résumé du contexte — synthèse globale du contexte et des objectifs',
  gbm_7a:
    'Parties prenantes — identification et cartographie des parties prenantes',
  gbm_7b:
    'Cartes des parties prenantes — relations donnant-donnant avec chaque partie prenante',
  ...
  gbm_12a:
    'Relations clients et canaux — stratégie de relation et distribution',
  gbm_12b: 'Parcours du client — expérience et parcours de chaque segment',
  gbm_13:
    'Activités et ressources clés — identification des activités et ressources critiques',
  gbm_14a: "Écoconception — conception durable du modèle d'affaires",
  gbm_14b:
    "Résultats de l'écoconception — impacts obtenus par la stratégie d'écoconception",
  gbm_15: 'Résumé des activités — synthèse des activités et ressources',
  gbm_16:
    'Structure des coûts — analyse des coûts fixes, variables et seuil de rentabilité',
  ...
  gbm_20:
    'Indicateurs — définition des KPIs environnementaux, sociaux et économiques',
  gbm_21:
    'Analyse SWOT — forces, faiblesses, opportunités et menaces synthétisées',
};
...
export class ReformulationService {
  ...
  async reformulateStep(
    ...
    const concept =
      STEP_CONCEPTS[stepKey] || `Étape ${stepKey} du business model`;

    if (!stepData || !stepData.content) {
      throw new Error(
        `Aucune donnée trouvée pour l'étape ${stepKey} du projet ${projectId}`,
      );
    }

    const audienceInstructions: Record<string, string> = {
      debutant:
        "Utilise un langage simple et accessible. Explique les concepts comme si tu t'adressais à un débutant complet. Évite le jargon technique. Ajoute des analogies concrètes.",
      intermediaire:
        "Utilise un langage clair mais précis. Garde la terminologie métier tout en t'assurant que c'est compréhensible. Ajoute des exemples pertinents.",
      avance:
        "Utilise un langage technique et précis. Tu t'adresses à un expert. Approfondis les concepts et ajoute des nuances stratégiques.",
    };
...
Retourne UNIQUEMENT un objet JSON avec les clés : reformulated_text, key_strengths (array), improvement_axes (array), reflection_questions (array)`;
    ...
    try {
      ...
      parsed = {
        reformulated_text: response.content,
        key_strengths: [],
        improvement_axes: [],
  ...
  async reformulateText(
    ...
    const audienceInstructions: Record<string, string> = {
      ...
      intermediaire:
        'langage clair avec terminologie métier, adapté à un entrepreneur averti',
      avance:
        'langage technique et précis, adapté à un expert en entrepreneuriat',
    };
...
- Maximum 300 mots`;
  ...
  private async fetchStepData(
    ...
    if (!modelName) {
      return {
        content: `Données de l'étape ${stepKey} (non mappée à un modèle spécifique)`,
      };
    }
    ...
    const rowFields = (row: Record<string, unknown>): string => {
      const fields = Object.keys(row).filter(
        (k) => !['id', 'project_id', 'created_at', 'updated_at'].includes(k),
      );
      return fields
        .map((f) => `${f.replace(/_/g, ' ')} : ${row[f] || 'Non renseigné'}`)
        .join('\n');
    };
