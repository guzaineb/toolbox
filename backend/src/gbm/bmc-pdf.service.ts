  ...
  private buildPdf(project: any, data: any): Promise<Buffer> {
    return new Promise((resolve, reject) => {
      ...
      const drawBox = (
        ...
        if (textH > 10) {
          ...
          for (const field of fields) {
            ...
            const truncated =
              val.length > maxLen ? val.substring(0, maxLen - 3) + '...' : val;
      ...
      doc
        ...
        .font('Helvetica')
        .fillColor(ink2)
        .text(`BMC — ${project.name} — 1/1`, ml, doc.page.height - 18, {
          width: pw,
          align: 'center',
        });
  ...
  private getBmcBlocks(data: any): BmcBlock[] {
    ...
    return [
      ...
      },
      {
        ...
        fields: [
          ...(vp
            ? [
                {
                  label: 'Valeur environnementale',
                  value: fmt(vp.environmental_value),
                },
                { label: 'Valeur sociale', value: fmt(vp.social_value) },
                {
                  label: 'Soulagement des douleurs',
                  value: fmt(vp.pain_relievers),
                },
                { label: 'Créateurs de gains', value: fmt(vp.gain_creators) },
                {
                  label: 'Produits et services',
                  value: fmt(vp.products_services),
                },
                { label: 'Valeur ajoutée', value: fmt(vp.value_added) },
                {
                  label: "Valeur d'innovation",
                  value: fmt(vp.innovation_value),
                },
              ]
            : [{ label: 'Proposition de valeur', value: '—' }]),
          ...(data.value_proposition_pivot &&
          data.value_proposition_pivot.new_value_proposition
            ? [
                {
                  label: 'Nouvelle proposition (pivot)',
                  value: fmt(
                    data.value_proposition_pivot.new_value_proposition,
                  ),
                },
        ...
        fields: crc
          ? [
              { label: 'Canaux', value: fmt(crc.channels) },
              {
                ...
                value: fmt(crc.distribution_strategy),
              },
            ]
          : [{ label: 'Canaux', value: '—' }],
      },
            ...
            ? [
                {
                  ...
                  value: fmt(crc.customer_relationships),
                },
              ]
            : [{ label: 'Relations clients', value: '—' }]),
          ...this.formatManyToOne(
        ...
        fields: rs
          ? [
              ...
              { label: 'Stratégie de prix', value: fmt(rs.pricing_strategy) },
              {
                ...
                value: fmt(rs.revenue_projections),
              },
              ...(crs?.revenue_summary
                ? [
                    {
                      label: 'Résumé des revenus',
                      value: fmt(crs.revenue_summary),
                    },
                  ]
                : []),
              ...(crs?.financial_health
                ? [
                    {
                      label: 'Santé financière',
                      value: fmt(crs.financial_health),
                    },
                  ]
                : []),
            ]
      ...
      },
      {
        ...
        fields: [
          ...(kar
            ? [{ label: 'Activités clés', value: fmt(kar.key_activities) }]
            : []),
          ...(sa
            ? [
                {
                  label: 'Résumé des activités',
                  value: fmt(sa.activities_summary),
                },
          ...
          ...(data.test_preparation
            ? [
                {
                  label: 'Objectifs de test',
                  value: fmt(data.test_preparation.test_objectives),
                },
              ]
            : []),
        ],
      },
      {
        ...
        fields: [
          ...
          ...(kar?.strategic_partners
            ? [
                {
                  label: 'Partenaires stratégiques',
                  value: fmt(kar.strategic_partners),
                },
              ]
            : []),
        ...
        fields: cs
          ? [
              ...
              { label: 'Facteurs de coûts', value: fmt(cs.cost_drivers) },
              {
                ...
                value: fmt(cs.breakeven_analysis),
              },
              ...(crs?.cost_summary
                ? [{ label: 'Résumé des coûts', value: fmt(crs.cost_summary) }]
                : []),
            ]
  ...
  private getExtraSections(
    ...
    if (pn) {
      sections.push({
        ...
        fields: [
          {
            label: 'Défis environnementaux',
            value: pn.environmental_challenges || '—',
          },
        ...
        fields: [
          {
            ...
            value: obj.environmental_objectives || '—',
          },
        ...
        fields: [
          {
            label: 'Politique',
            value:
              p.political_what && p.political_how
                ? `${p.political_what} — ${p.political_how}`
                : '—',
          },
          {
            label: 'Économique',
            value:
              p.economic_what && p.economic_how
                ? `${p.economic_what} — ${p.economic_how}`
                : '—',
          },
          {
          ...
          },
          {
            label: 'Technologique',
            value:
              p.technological_what && p.technological_how
                ? `${p.technological_what} — ${p.technological_how}`
                : '—',
          },
          {
            ...
            value:
              p.environmental_what && p.environmental_how
                ? `${p.environmental_what} — ${p.environmental_how}`
                : '—',
          },
          {
      ...
      sections.push({
        ...
        fields: [
          { label: 'Résultats', value: edr.eco_results || '—' },
          {
            ...
            value: edr.performance_analysis || '—',
          },
  ...
  private getMonitoringSections(
    ...
    if (ind) {
      sections.push({
        ...
        fields: [
          {
            label: 'KPIs environnementaux',
            value: ind.environmental_kpis || '—',
          },
