...
export class EcoDesignService {
  ...
  async get(projectId: string, userId: string) {
    ...
    const record = await this.prisma.ecoDesign.findUnique({
  ...
  async update(projectId: string, data: any, userId: string) {
    ...
    return this.sections.saveSection(this.prisma.ecoDesign, projectId, data, {
      allowedFields: ECO_DESIGN_ALLOWED_FIELDS,
      stepKey: 'eco_design',
    });
  }
  ...
  async getProgress(projectId: string, userId: string) {
    ...
    const phases = [
      {
        name: 'Préparer la valise',
        fields: ['equipe_eco', 'projet_eco', 'contexte_eco', 'vision_durable'],
        completed: 0,
      },
      {
        name: 'Configurer le cycle de vie',
        fields: ['cycle_de_vie'],
        completed: 0,
      },
      {
        name: 'Évaluer la performance',
        fields: ['performance_eco'],
        completed: 0,
      },
      {
        name: 'Choix & évaluation stratégie',
        fields: ['strategies_eco'],
        completed: 0,
      },
    ...
    for (const phase of phases) {
      phase.completed = phase.fields.filter(
        (f: string) => (record as any)[f] != null,
      ).length;
    }
